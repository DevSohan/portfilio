...
function ExperienceCard({info}: Props) {
    ...
    return (
        <div className={`flex flex-col justify-center items-center py-3 w-full md:w-1/2 ${iconPositionClass} md:space-x-0 md:px-8 md:justify-between md:items-start md:gap-x-6 lg:gap-x-10 ${alignClass}`}>
            <div className='w-12 rounded-full bg-dev-sohan-black-2 p-2 ring-4 ring-black ring-inset'>
                <AcademicCapIcon />
            </div>
            <div className={`bg-dev-sohan-black-2 text-white antialiased my-5 p-4 rounded-lg text-center md:text-left md:my-0 md:relative md:w-4/5 before:md:content-[""] before:md:border-8 before:md:border-solid before:md:border-transparent before:md:h-4 before:md:w-4 before:md:absolute ${hoverborderClass}`}>
                ...
                <div className='text-xs'>
                    <div className='flex flex-row gap-x-3 mt-4'>
                    {
                        info.skills.map((skill, key) => {
                            return(
                                <span className='py-1 px-2 bg-yellow-300 space-x-1' key={key}>{skill}</span>
                            )
                        })
                    }
                    </div>
